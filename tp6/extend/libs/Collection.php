...
class Collection implements ArrayAccess, Countable, IteratorAggregate, JsonSerializable
{
    ...
    public function channelLevel(int $pid = 0 , string $fieldPri = 'id' , string $fieldPid = 'pid' , int $level = 1)
    {
        if (empty($this->items)) {
            return [];
        }
        ...
        foreach ($this->items as $v) {
            if ($v[$fieldPid] == $pid) {
                ...
                $arr[$v[$fieldPri]]["_son"] = self::channelLevel($v[$fieldPri] , $fieldPri , $fieldPid , $level + 1)->toArray();
            }
        }
