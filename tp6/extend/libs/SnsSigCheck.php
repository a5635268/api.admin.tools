...
class SnsSigCheck
{
     ...
     * @return string
     */
    static public function makeSig(string $method, string $url_path, array $params, string $secret):string
    {
    ...
    static private function makeSource(string $method, string $url_path, array $params):string
    {
     ...
     * @return bool
     */
    static public function verificationData(array $data):array
    {
        if(empty($data['_sign'])){
            return false;
        }
        $apiSecret = $data['_sign'];
        unset($data['_sign']);
        krsort($data);
